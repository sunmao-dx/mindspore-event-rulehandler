...
def is_promoted(owner_id, issue_operate_logs, issue_comments):
    ...
    mindspore_dx_bot_id = 8777557
    mindspore_ci_bot_id = 5518576
    mindspore_irobot_id = 7718575

    bot_list = (mindspore_dx_bot_id, mindspore_ci_bot_id, mindspore_irobot_id)
    # 日志里是否有推进issue解决的正面行为
    for action in issue_operate_logs:
        ...
        if action_owner_id == owner_id and (action_icon != 'add user icon' and action_icon != 'tag icon'):
            other_flag = True
        # 有没有其他人（非bot）通过修改issue信息帮助解决的情况
        if action_owner_id != owner_id and action_owner_id not in bot_list:
            helper_flag = True

    # 评论区里是否有推进issue解决的正面行为
    ...
    for comment in issue_comments:
        ...
        if comment_owner_id == owner_id and comment['body'][0] != '/':
            other_flag = True
        # 有没有其他人（非bot）通过评论帮助解决的情况
        if comment_owner_id != owner_id and comment_owner_id not in bot_list:
            helper_flag = True

    total_flag = label_flag or assign_flag or other_flag

    return total_flag, label_flag, assign_flag, helper_flag, other_flag
